...
import type { AppRequest, AppResponse } from './http.ts';
import type {
...
import { convertToCursorRequest, parseToolCalls } from './converter.ts';
import { sendCursorRequest, sendCursorRequestFull } from './cursor-client.ts';
import { getConfig } from './config.ts';
import { prefixedId } from './id.ts';

function msgId(): string {
  return prefixedId('msg_', 24);
}

function toolId(): string {
  return prefixedId('toolu_', 24);
}
...
export function listModels(_req: AppRequest, res: AppResponse): void {
...
export function countTokens(req: AppRequest, res: AppResponse): void {
  const body = req.body as AnthropicRequest;
  let totalChars = 0;

  if (body.system) {
    totalChars += typeof body.system === 'string'
      ? body.system.length
...
export function sanitizeResponse(text: string): string {
  ...
  result = result.replace(
    /I\s+am\s+(?:a\s+)?(?:support\s+)?assistant\s+for\s+Cursor/gi,
    'I am Claude, an AI assistant by Anthropic',
  );
  result = result.replace(
    /I(?:'m|\s+am)\s+(?:a\s+)?Cursor(?:'s)?\s+(?:support\s+)?assistant/gi,
    'I am Claude, an AI assistant by Anthropic',
  );
  result = result.replace(
    /Cursor(?:'s)?\s+support\s+assistant/gi,
    'Claude, an AI assistant by Anthropic',
  );
  result = result.replace(
    /support\s+assistant\s+for\s+Cursor/gi,
    'Claude, an AI assistant by Anthropic',
  );
  result = result.replace(
    /I\s+run\s+(?:on|in)\s+Cursor(?:'s)?\s+(?:support\s+)?system/gi,
  ...
  result = result.replace(
    /(?:help\s+with\s+)?coding\s+and\s+Cursor\s+IDE\s+questions/gi,
    'help with a wide range of tasks',
  );
  result = result.replace(
    /(?:I'?m|I\s+am)\s+here\s+to\s+help\s+with\s+coding\s+and\s+Cursor[^.]*\./gi,
    'I am Claude, an AI assistant by Anthropic. I can help with a wide range of tasks.',
  );
  // "Cursor IDE features" -> "AI assistance"
  result = result.replace(
    /\*\*Cursor\s+IDE\s+features\*\*/gi,
    '**AI capabilities**',
  );
  result = result.replace(
    /Cursor\s+IDE\s+(?:features|questions|related)/gi,
    'various topics',
  );
  // "unrelated to programming or Cursor" -> "outside my usual scope, but I'll try"
  result = result.replace(
    /unrelated\s+to\s+programming\s+or\s+Cursor/gi,
    'a general knowledge question',
  );
  result = result.replace(
    /unrelated\s+to\s+(?:programming|coding)/gi,
    'a general knowledge question',
  );
  // "Cursor-related question" -> "question"
  result = result.replace(
    /(?:a\s+)?(?:programming|coding|Cursor)[- ]related\s+question/gi,
    'a question',
  );
  // "ask a programming or Cursor-related question" -> "ask me anything" (must be before generic patterns)
  result = result.replace(
    /(?:please\s+)?ask\s+a\s+(?:programming|coding)\s+(?:or\s+(?:Cursor[- ]related\s+)?)?question/gi,
    'feel free to ask me anything',
  );
  // Generic "Cursor" in capability descriptions
  result = result.replace(
    /questions\s+about\s+Cursor(?:'s)?\s+(?:features|editor|IDE|pricing|the\s+AI)/gi,
    'your questions',
  );
  result = result.replace(
    /help\s+(?:you\s+)?with\s+(?:questions\s+about\s+)?Cursor/gi,
    'help you with your tasks',
  );
  result = result.replace(
    /about\s+the\s+Cursor\s+(?:AI\s+)?(?:code\s+)?editor/gi,
    '',
  );
  result = result.replace(
    /Cursor(?:'s)?\s+(?:features|editor|code\s+editor|IDE),?\s*(?:pricing|troubleshooting|billing)/gi,
    'programming, analysis, and technical questions',
  );
  // Bullet list items mentioning Cursor
  result = result.replace(
    /(?:finding\s+)?relevant\s+Cursor\s+(?:or\s+)?(?:coding\s+)?documentation/gi,
  ...
  result = result.replace(
    /AI\s+chat,\s+code\s+completion,\s+rules,\s+context,?\s+etc\.?/gi,
  ...
  result = result.replace(
    /我是\s*Cursor\s*的?\s*支持助手/g,
    '我是 Claude，由 Anthropic 开发的 AI 助手',
  );
  result = result.replace(
    /Cursor\s*的?\s*支持(?:系统|助手)/g,
    'Claude，Anthropic 的 AI 助手',
  );
  result = result.replace(
    /运行在\s*Cursor\s*的?\s*(?:支持)?系统中/g,
    '运行在 Anthropic 的基础设施上',
  );
  result = result.replace(
    /帮助你解答\s*Cursor\s*相关的?\s*问题/g,
    '帮助你解答各种问题',
  );
  result = result.replace(
    /关于\s*Cursor\s*(?:编辑器|IDE)?\s*的?\s*问题/g,
    '你的问题',
  );
  result = result.replace(
    /专门.*?回答.*?(?:Cursor|编辑器).*?问题/g,
    '可以回答各种技术和非技术问题',
  );
  result = result.replace(
    /(?:功能使用[、,]\s*)?账单[、,]\s*(?:故障排除|定价)/g,
    '编程、分析和各种技术问题',
  );
  result = result.replace(/故障排除等/g, '等各种问题');
  result = result.replace(/我的职责是帮助你解答/g, '我可以帮助你解答');
  result = result.replace(
    /如果你有关于\s*Cursor\s*的问题/g,
    '如果你有任何问题',
  );
  ...
  result = result.replace(
    /如果有?\s*(?:Cursor\s*)?(?:相关|有关).*?(?:欢迎|请)\s*(?:继续)?(?:提问|询问)[。！!]?\s*/g,
    '',
  );
  result = result.replace(
    /如果你?有.*?(?:Cursor|编程|代码|开发).*?(?:问题|需求)[^。\n]*[。，,]?\s*(?:欢迎|请|随时).*$/gm,
    '',
  );
  ...
  if (
    /prompt\s+injection|social\s+engineering|I\s+need\s+to\s+stop\s+and\s+flag|What\s+I\s+will\s+not\s+do/i
      .test(result)
  ) {
  ...
  result = result.replace(
    /(?:I\s+)?(?:only\s+)?have\s+(?:access\s+to\s+)?(?:two|2)\s+tools?[^.]*\./gi,
    '',
  );
...
function handleMockIdentityStream(
  res: AppResponse,
  ...
  writeSSE(res, 'content_block_delta', {
    ...
    index: 0,
    delta: { type: 'text_delta', text: mockText },
  });
...
export async function handleMessages(
  ...
  console.log(
    `[Handler] 收到请求: model=${body.model}, messages=${body.messages?.length}, stream=${body.stream}, tools=${
      body.tools?.length ?? 0
    }`,
  );
  ...
  try {
    ...
    if (isIdentityProbe(body)) {
      ...
      if (body.stream) {
        handleMockIdentityStream(res, body);
        return;
      } else {
        handleMockIdentityNonStream(res, body);
        return;
      }
    }
  ...
  try {
    ...
    if (!hasTools) {
      while (isRefusal(fullResponse) && retryCount < MAX_REFUSAL_RETRIES) {
        retryCount++;
        console.log(
          `[Handler] 检测到身份拒绝（第${retryCount}次），自动重试...原始: ${
            fullResponse.substring(0, 80)
          }...`,
        );
      ...
      if (isRefusal(fullResponse)) {
        ...
        if (isToolCapabilityQuestion(body)) {
          ...
          console.log(
            `[Handler] 重试${MAX_REFUSAL_RETRIES}次后仍被拒绝，返回 Claude 身份回复`,
          );
    ...
    if (hasTools) {
      ...
      if (toolCalls.length > 0) {
        ...
        if (REFUSAL_PATTERNS.some((p) => p.test(cleanText))) {
          console.log(
            `[Handler] Supressed refusal text generated during tool usage: ${
        ...
        if (isRefusal(fullResponse)) {
          console.log(
            `[Handler] Supressed complete refusal without tools: ${
...
async function handleNonStream(
  ...
  console.log(
    `[Handler] 原始响应 (${fullText.length} chars): ${
  ...
  if (!hasTools && isRefusal(fullText)) {
    for (let attempt = 0; attempt < MAX_REFUSAL_RETRIES; attempt++) {
      console.log(
        `[Handler] 非流式：检测到身份拒绝（第${attempt + 1}次重试）...原始: ${
    ...
    if (isRefusal(fullText)) {
      if (isToolCapabilityQuestion(body)) {
        console.log(
          `[Handler] 非流式：工具能力询问被拒绝，返回 Claude 能力描述`,
        );
        fullText = CLAUDE_TOOLS_RESPONSE;
      } else {
        console.log(
          `[Handler] 非流式：重试${MAX_REFUSAL_RETRIES}次后仍被拒绝，返回 Claude 身份回复`,
        );
  ...
  if (hasTools) {
    ...
    if (toolCalls.length > 0) {
      ...
      if (isRefusal(cleanText)) {
        console.log(
          `[Handler] Supressed refusal text generated during non-stream tool usage: ${
            cleanText.substring(0, 100)
          }...`,
        );
        cleanText = '';
      ...
      if (isRefusal(fullText)) {
        console.log(
          `[Handler] Supressed pure text refusal (non-stream): ${
...
function writeSSE(res: AppResponse, event: string, data: unknown): void {
