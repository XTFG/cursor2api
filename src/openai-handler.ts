...
function chatId(): string {
  return prefixedId('chatcmpl-', 24);
}

function toolCallId(): string {
  return prefixedId('call_', 24);
}
 ...
 */
function extractOpenAIContentBlocks(
  msg: OpenAIMessage,
...
export async function handleOpenAIChatCompletions(
  ...
  console.log(
    `[OpenAI] 收到请求: model=${body.model}, messages=${body.messages?.length}, stream=${body.stream}, tools=${
      body.tools?.length ?? 0
    }`,
  );
...
function handleOpenAIMockStream(
  ...
  writeOpenAISSE(res, {
    ...
    choices: [{
      index: 0,
      delta: { role: 'assistant', content: mockText },
      finish_reason: null,
    }],
  });
  writeOpenAISSE(res, {
...
function handleOpenAIMockNonStream(
  res: AppResponse,
  body: OpenAIChatRequest,
  mockText: string,
): void {
  ...
  try {
    ...
    if (!hasTools) {
      while (isRefusal(fullResponse) && retryCount < MAX_REFUSAL_RETRIES) {
        retryCount++;
        console.log(
          `[OpenAI] 检测到拒绝（第${retryCount}次），自动重试...原始: ${
      ...
      if (isRefusal(fullResponse)) {
        if (isToolCapabilityQuestion(anthropicReq)) {
          ...
          console.log(
            `[OpenAI] 重试${MAX_REFUSAL_RETRIES}次后仍被拒绝，返回 Claude 身份回复`,
          );
...
async function handleOpenAINonStream(
  ...
  console.log(
    `[OpenAI] 原始响应 (${fullText.length} chars): ${
  ...
  if (!hasTools && isRefusal(fullText)) {
    for (let attempt = 0; attempt < MAX_REFUSAL_RETRIES; attempt++) {
      console.log(
        `[OpenAI] 非流式：检测到拒绝（第${attempt + 1}次重试）...原始: ${
    ...
    if (isRefusal(fullText)) {
      if (isToolCapabilityQuestion(anthropicReq)) {
        console.log(
          `[OpenAI] 非流式：工具能力询问被拒绝，返回 Claude 能力描述`,
        );
        fullText = CLAUDE_TOOLS_RESPONSE;
      } else {
        console.log(
          `[OpenAI] 非流式：重试${MAX_REFUSAL_RETRIES}次后仍被拒绝，返回 Claude 身份回复`,
        );
