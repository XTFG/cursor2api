...
import { applyVisionInterceptor } from './vision.ts';
 ...
 */
export async function convertToCursorRequest(
  req: AnthropicRequest,
): Promise<CursorChatRequest> {
  ...
  if (req.system) {
    if (typeof req.system === 'string') combinedSystem = req.system;
      ...
      combinedSystem = req.system.filter((b) => b.type === 'text').map((b) =>
        b.text
      ).join('\n');
    }
  }
  ...
  if (hasTools) {
    ...
    const hasCommunicationTool = tools.some((t) =>
    ...
    const readTool = tools.find((t) =>
      /^(Read|read_file|ReadFile)$/i.test(t.name)
    );
    const bashTool = tools.find((t) =>
      /^(Bash|execute_command|RunCommand)$/i.test(t.name)
    );
    ...
    const fewShotParams = fewShotTool.name.match(/^(Read|read_file|ReadFile)$/i)
      ...
      ? Object.fromEntries(
        Object.entries(
          fewShotTool.input_schema.properties as Record<
            string,
            { type?: string }
          >,
        )
      ...
      parts: [{
        ...
        text:
          `Understood. I'll use the structured format for actions. Here's how I'll respond:\n\n\`\`\`json action\n${
            JSON.stringify(
              { tool: fewShotTool.name, parameters: fewShotParams },
              null,
              2,
            )
          }\n\`\`\``,
    ...
    for (let i = 0; i < req.messages.length; i++) {
      ...
      if (msg.role === 'user') {
        ...
        while (processTags()) {
          // Continue consuming structured blocks until no more tags remain.
        }
    ...
    for (const msg of req.messages) {
      ...
      if (msg.role === 'assistant') {
        if (
          /Cursor(?:'s)?\s+support\s+assistant|I\s+only\s+answer|read_file|read_dir|I\s+cannot\s+help\s+with|文档助手|只有.*两个.*工具|工具仅限于/i
            .test(text)
        ) {
    ...
    switch (block.type) {
      ...
      case 'image':
        ...
        if (block.source?.data) {
          ...
          parts.push(
            `[Image attached: ${mediaType}, ~${sizeKB}KB. Note: Image was not processed by vision system. The content cannot be viewed directly.]`,
          );
          console.log(
            `[Converter] ❗ 图片块未被 vision 预处理掉，已添加占位符 (${mediaType}, ~${sizeKB}KB)`,
          );
        } else {
      ...
      case 'tool_result': {
        ...
        if (
          block.is_error &&
          /haven't\s+granted|not\s+permitted|permission|unauthorized/i.test(
            resultText,
          )
        ) {
          ...
          parts.push(
            `${prefix} (tool_use_id: ${block.tool_use_id}):\n${resultText}`,
          );
        }
        break;
      }
...
async function preprocessImages(messages: AnthropicMessage[]): Promise<void> {
  ...
  console.log(
    `[Converter] 📸 检测到 ${totalImages} 张图片，启动 vision 预处理...`,
  );
  ...
  try {
    ...
    if (remainingImages > 0) {
      console.log(
        `[Converter] ⚠️ vision 处理后仍有 ${remainingImages} 张图片未被替换（可能 vision.enabled=false 或处理失败）`,
      );
    } else {
      console.log(
        `[Converter] ✅ 全部 ${totalImages} 张图片已成功处理为文本描述`,
      );
    }
  } catch (err) {
